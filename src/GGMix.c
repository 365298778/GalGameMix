...
static		int		render_update(){
	if(!inited){
		bmp		=	bmp_func.InitFile(ggmix,"BG001.bmp");
		inited	=	1;
	}
	//ggmix->DrawBMPFile(ggmix,"BG001.bmp",0,0,800,600);
//	w+=wv;
//	if(w<0)
//		if(wv<0)
//			wv = 10;
...
//			wv=-10;
	...
	//bmp_func.DrawBMPAlpha(ggmix,bmp,0,0,w,600,0,0,w,600,0x80);
	int y;
	int x;
...
#ifdef		TEST_
	ggmix->RenderUpdate		=	render_update;		//渲染更新
	ggmix->Closing			=	(void*)closing;			//关闭时间
#else
#ifdef		DEBUG_TEST_
	ggmix->RenderUpdate		=	test_render_update;
	ggmix->Closing			=	test_closing;
	ggmix->Update			=	test_update;
#endif
#endif		/*TEST_*/
